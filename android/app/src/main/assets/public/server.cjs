...
var generateADHDStepFallback = (task) => {
...
Remember: You aren't lazy or behind. Starting is just a chemical spark threshold. Give yourself permission to make micro-progress! \u{1F33F}\u2728`;
};
var generateEmailDraftFallback = (template, situation, tone) => {
  ...
  if (isHR) {
    const subject2 = "Professional Development Sponsorship Request: FlowHer\u2122 Core Subscription";
    const greeting2 = "Dear [HR Partner/Manager Name],";
    const body2 = `I am writing to formally request sponsorship or benefit reimbursement for an annual workspace subscription to FlowHer\u2122 Core. FlowHer\u2122 is a highly specialized executive-functioning and communication optimization platform designed for neurodivergent professionals (ADHD/Autism).
...
var generateScriptFallback = (scriptType, situation) => {
...
var generateRSDCheckFallback = (spiral) => {
...
var generateMeetingPrepFallback = (topic, people, goal, anxietyLevel) => {
...
app.get("/get-source-code", (req, res) => {
  ...
  try {
    const singleFileHtmlPath = import_path.default.join(process.cwd(), "index_single_file_live.html");
    if (import_fs.default.existsSync(singleFileHtmlPath)) {
      const code = import_fs.default.readFileSync(singleFileHtmlPath, "utf8");
      res.send(`
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          ...
          <div class="card">
            ...
            <div class="actions">
              <button class="btn" id="copyBtn">\u{1F4CB} Copy Code to Clipboard</button>
              <a class="btn btn-secondary" href="/get-source-code/raw" download="index.html">\u{1F4E5} Download index.html</a>
            </div>
          ...
          <script>
            ...
            function showSuccess() {
              ...
              setTimeout(() => {
                ...
                copyBtn.innerText = '\u{1F4CB} Copy Code to Clipboard';
              }, 3000);
            }
...
app.get("/get-react-code", (req, res) => {
  res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0, post-check=0, pre-check=0");
  try {
    const reactAppPath = import_path.default.join(process.cwd(), "src", "App.tsx");
    const singleFileHtmlPath = import_path.default.join(process.cwd(), "index_single_file_live.html");
    let reactCode = "";
    if (import_fs.default.existsSync(reactAppPath)) {
      reactCode = import_fs.default.readFileSync(reactAppPath, "utf8");
    }
    let compiledHtml = "";
    if (import_fs.default.existsSync(singleFileHtmlPath)) {
      compiledHtml = import_fs.default.readFileSync(singleFileHtmlPath, "utf8");
    }
    res.send(`
      <!DOCTYPE html>
      <html>
      ...
      <body>
        ...
        <div class="card">
          ...
          <div class="tab-container">
            <button class="tab active" onclick="switchTab('html')">\u{1F31F} Compiled Single-File HTML (For flowherapp.com)</button>
            <button class="tab" onclick="switchTab('tsx')">\u{1F4BB} React App.tsx Source Code</button>
          </div>
          ...
          <div id="panel-html" class="panel active">
            ...
            <div class="actions">
              <button class="btn" onclick="copyContent('htmlCode', 'htmlBtn')">\u{1F4CB} Copy Compiled HTML Code</button>
              <a class="btn btn-secondary" href="/get-source-code/raw" target="_blank" download="index.html">\u{1F4E5} Download index.html</a>
            </div>
          ...
          <div id="panel-tsx" class="panel">
            ...
            <div class="actions">
              <button class="btn" onclick="copyContent('tsxCode', 'tsxBtn')">\u{1F4CB} Copy App.tsx Code</button>
              <a class="btn btn-secondary" href="/get-react-code/raw" target="_blank">\u{1F4C4} View Raw App.tsx</a>
            </div>
